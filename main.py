...
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "")  # Бесплатный Groq API
...
MAX_POSTS_PER_CYCLE = 1  # По одной новости за запуск
...
def analyze_text_with_groq(title, text):
    """Анализирует статью через Groq API (БЕСПЛАТНО) и делает краткий вывод."""
    if not GROQ_API_KEY:
        print(f"      ⚠️  Groq API не настроен, просто сокращаю текст")
...
- Без излишних деталей"""
        ...
        if resp.ok:
            ...
            print(f"      ⚠️  Groq ошибка {resp.status_code}")
    except Exception as e:
...
def format_post(entry, source_name):
    ...
    # Анализируем текст через Groq
    print(f"      📝 Анализирую новость (Groq)...")
    analysis = analyze_text_with_groq(title, summary)
...
def main():
    ...
    if not GROQ_API_KEY:
        print("⚠️  ВНИМАНИЕ: GROQ_API_KEY не установлен, буду использовать простое сокращение текста")
    
    if not REPLICATE_API_KEY:
        print("⚠️  ВНИМАНИЕ: REPLICATE_API_KEY не установлен, картинок генерироваться не будет")
