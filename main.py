...
def format_post(entry, source_name):
    title = html.escape(entry.get("title", "Без заголовка")[:150])
    link = entry.get("link", "")
    summary = entry.get("summary", "")
    # Убираем HTML-теги и HTML-сущности
    summary = re.sub("<[^<]+?>", "", summary)
    summary = re.sub("&nbsp;", " ", summary)  # Убираем &nbsp;
    summary = re.sub("&[a-z]+;", "", summary)  # Убираем другие HTML-сущности
    summary = html.escape(summary[:1200]).strip()  # Полный текст, максимум 1200 символов
