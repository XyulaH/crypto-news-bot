...
def normalize_link(link):
    """Нормализует ссылку - убирает utm параметры и трекинг."""
    if not link:
        return link
    
    # Убираем utm параметры и другой трекинг
    link = re.sub(r'[?&]utm_\w+=[^&]*', '', link)
    link = re.sub(r'[?&]ref=\w+', '', link)
    link = re.sub(r'[?&]t=\d+', '', link)
    link = re.sub(r'#.*$', '', link)  # убираем якоря
    
    # Убираем лишние ? если остались
    link = link.rstrip('?&')
...
def search_image_unsplash(query):
    """Ищет картинку на Unsplash (с random для разных картинок)."""
    try:
        url = "https://api.unsplash.com/search/photos"
        # Добавляем random параметр чтобы избежать кеша
        import random
        params = {
            "query": query,
            "per_page": 3,  # Берем 3 варианта
            "page": random.randint(1, 5),  # Разные страницы результатов
        ...
        if resp.ok:
            data = resp.json()
            if data.get("results"):
                # Берем случайный результат из полученных
                random_idx = random.randint(0, min(2, len(data["results"]) - 1))
                image_url = data["results"][random_idx].get("urls", {}).get("regular")
                if image_url:
                    return image_url
    except:
        pass
...
def search_image_pexels(query):
    """Ищет картинку на Pexels (с разными результатами)."""
    try:
        ...
        if resp.ok:
            data = resp.json()
            if data.get("photos"):
                random_idx = random.randint(0, min(2, len(data["photos"]) - 1))
                return data["photos"][random_idx].get("src", {}).get("large")
    except:
        pass
    ...
    for feed_url in RSS_FEEDS:
        ...
        try:
            ...
            for entry in feed.entries[:15]:
                ...
                # Нормализуем ссылку для проверки дубликатов
                ...
                if normalized_link in seen:
                    print(f"      ⏭️  Уже опубликовано (дубликат)")
                    continue
                ...
                if send_to_telegram(post_text, image_url):
                    ...
                    seen.add(normalized_link)  # Сохраняем нормализованную ссылку
                    new_count += 1
